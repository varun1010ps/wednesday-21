...
class TeamAccessView extends Component {
  ...
  render() {
    const { title, topic, description, theme1, imageUrl, audioFile} = this.props.story;
    return (
...
const mapStateToProps = (state, ownProps) => {
  return { story: state.story[ownProps.match.params.id] };
};

export default connect(mapStateToProps, { fetchStory })(TeamAccessView);
